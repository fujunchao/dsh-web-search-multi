...
`;
    ...
    function SearchSettingsCard({ scope, api, t }) {
      ...
      React.useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
          try {
            ...
            const view = response.result.value.credentials[credentialRef];
            setCredential({
              ref: credentialRef,
      ...
      }, [api, credentialRef, credentialRefresh]);
      ...
      const save = async () => {
        ...
        try {
          for (const field of changed) await scope.set(field.key, draftValue(field, draft));
          const accepted = normalizedConfig(scope.getSnapshot().value);
          if (changed.some((field) => !sameDraftValue(field, accepted[field.key], draft[field.key]))) {
          ...
          for (const entry of PROVIDERS) {
            ...
            const response = await api.credentials.set({ ref, value: key });
            if (!response.result.ok) throw new Error("credential write was rejected");
          }
          setDraft(accepted);
          setSecretDrafts({ tavily: "", grok: "", openai: "", gemini: "" });
          setTouched(false);
