...
const Config = z.object({
  ...
  tavilyApiKeyEnv: z.string().role("credential-ref").default(DEFAULTS.tavilyApiKeyEnv),
  tavilyBaseURL: z.string().default(DEFAULTS.tavilyBaseURL),
  tavilyMaxResults: z.number().step(1).min(1).default(DEFAULTS.tavilyMaxResults),
  tavilySearchDepth: z.string().default(DEFAULTS.tavilySearchDepth),
  tavilyIncludeAnswer: z.boolean().default(DEFAULTS.tavilyIncludeAnswer),
  grokApiKey: z.string().role("secret"),
  grokApiKeyEnv: z.string().role("credential-ref").default(DEFAULTS.grokApiKeyEnv),
  grokBaseURL: z.string().default(DEFAULTS.grokBaseURL),
  grokModel: z.string().default(DEFAULTS.grokModel),
  openaiApiKey: z.string().role("secret"),
  openaiApiKeyEnv: z.string().role("credential-ref").default(DEFAULTS.openaiApiKeyEnv),
  openaiBaseURL: z.string().default(DEFAULTS.openaiBaseURL),
  openaiModel: z.string().default(DEFAULTS.openaiModel),
  geminiApiKey: z.string().role("secret"),
  geminiApiKeyEnv: z.string().role("credential-ref").default(DEFAULTS.geminiApiKeyEnv),
  geminiBaseURL: z.string().default(DEFAULTS.geminiBaseURL),
  geminiModel: z.string().default(DEFAULTS.geminiModel),
});

const MULTI_SETTINGS_NAMESPACE = settingsNamespace("web-search-multi");
...
class MultiSearchProvider {
  ...
  async searchGemini(request, options, signal) {
    ...
    try {
      ...
      throw new WebError(`Gemini search request failed: ${String(error)}`, "WEB_PROVIDER_ERROR", { cause: error });
    }
...
function apply(ctx, config) {
  ...
  let current = () => config;
  installSettingsSection(ctx, MULTI_SETTINGS_NAMESPACE, Config, config, {
    setSource: (source) => {
      current = source;
    },
    onChange: () => {},
  });
  ctx.web.registerSearchProvider(new MultiSearchProvider(() => resolveOptions(ctx, current())));
}
